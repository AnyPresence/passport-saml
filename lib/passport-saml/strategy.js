...
Strategy.prototype.authenticate = function (req, options) {
  ...
  function redirectIfSuccess(err, url, request) {
    if (err) return self.error(err);

    var res = req.res;
    res.send(200, { message: 'You are being redirected', redirectUrl: url, SAMLRequest: request.SAMLRequest, SigAlg: request.SigAlg, Signature: request.Signature });
  }
