...
SAML.prototype.getAuthorizePayload = function(req, callback) {
  var self = this;
  ...
  self.generateAuthorizeRequest(req, self.options.passive, function(err, request) {
    ...
    var target = url.parse(self.options.entryPoint, true);
    if (operation !== 'authorize') return callback(new Error('Unknown operation: ' + operation));

    delete target.search;

    // XML version tag needs to be removed for post request to work properly.
    request = request.replace('<?xml version="1.0"?>', '');

    var bufferRequest = new Buffer(request, 'utf-8');
    var base64Request = bufferRequest.toString('base64');

    requestObj.SAMLRequest = base64Request;

    if (self.options.privateCert) {
      var signing = self.signRequestBody(request);
      requestObj.Signature = signing.Signature;
      requestObj.SigAlg = signing.SigAlg;
    }

    callback(null, url.format(target), requestObj);
  });
};

SAML.prototype.signRequestBody = function(request) {
  var signing = {};

  var signer;
  switch(this.options.signatureAlgorithm) {
    case 'sha256':
      signing.SigAlg = 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha256';
      signer = crypto.createSign('RSA-SHA256');
      break;
    default:
      signing.SigAlg = 'http://www.w3.org/2000/09/xmldsig#rsa-sha1';
      signer = crypto.createSign('RSA-SHA1');
      break;
  }
  signer.update(request);
  signing.Signature = signer.sign(this.options.privateCert, 'base64');
  return signing;
};
